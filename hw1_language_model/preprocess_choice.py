...
class Model(object):
	def __init__(self):
		...
		self.th_infrequent = 8000 # only process this number of frequent words
	...
	def pickle_corpus(self, save_pickle):
		...
		if save_pickle == 1: pickle.dump(self.corpus, open("../model/corpus", "wb"))
		elif save_pickle == 2:
			self.corpus = pickle.load(open("../model/corpus", "rb"))

	def pickle_freq_corpus(self, save_pickle):
		print "Pickle frequent corpus"
		if save_pickle == 1: pickle.dump(self.corpus, open("../model/freq_corpus", "wb"))
	...
	def add_common(self):
		...
		for i, line in enumerate(fin):
			...
			if l[1] not in self.common1: self.common1.append(l[1])
		for i in range(len(self.common1)): print self.common1[i]
