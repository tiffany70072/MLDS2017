...
class Model(object):
	...
	def merge(self):
		...
		for i, line in enumerate(f):
			...
			for j in range(n): self.conv_id.append([str(l[2*j+1]), str(l[2*j+3])])
	...
	def build_line_dict(self):
		...
		#for i in range(5): print self.line_dict[self.conv_id[i][0]], self.line_dict[self.conv_id[i][1]]
	...
	def change_to_line(self):
		...
		'''for i in range(len(self.conv)):
			for j in range(2): print self.conv[i][j]
			print ""
		'''
	...
	def count(self):
		word_set = []
		for i in range(len(self.corpus)): 
			for j in range(2): word_set += self.corpus[i][j]
		...
		for i in range(len(commonTemp)): self.common.append(commonTemp[i][0])
	...
	def change_to_id(self, max_length):
		...
		for i in range(len(self.corpus)):
			...
			corpus[i][0][pad] = 1
			for k in range(n):
				try: corpus[i][0][pad+k+1] = self.word_id[self.corpus[i][0][k]]
				except KeyError: corpus[i][0][pad+k+1] = 3
			...
			corpus[i][1][0] = 1
			for k in range(n):
				try: corpus[i][1][k+1] = self.word_id[self.corpus[i][1][k]]
				except KeyError: corpus[i][1][k+1] = 3
			corpus[i][1][n+1] = 2

			# check
			for j in range(max_length-1):
				if corpus[i][1][j] == 3 and corpus[i][1][j+1] == 3: remove_list.append(i)
