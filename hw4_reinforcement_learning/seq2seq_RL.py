...
class Model(object):
	def __init__(self):
		...
		self.lam1 = 0.3 # dull set
		self.lam2 = 0.1 # EOS, Unknown
		self.lam3 = 0.6
		...
		self.total_epoch = int(10000) 
		
		self.sess = tf.Session()
		
	def load(self):
		self.x, self.y = seq2seq_lib.load("corpus_15.npy", self.train_size)
		self.id_word = pickle.load(open("../data/id_word_dict", "rb"))
		self.dull = np.load("../data/dull.npy")
	...
	def select_time(self, h):
		...
		#for i in range(1, self.h_time): temp = tf.concat([temp, h[:, i*self.n+self.n-1:(i+1)*self.n, :]], 1)
	...
	def initialize_model(self):
		...
		if self.first_train == 0:
			self.saver.restore(self.sess, "../model_seq2seq_512/model" + name + ".ckpt")
	...
	def testing(self):
		...
		pred = self.sess.run(self.pred_test, feed_dict = {
			self.x_nn: self.x_batch, self.bs: self.batch_size, self.keep_prob: self.drop})
		...
		for i in range(6):
			for j in range(self.num_steps): print self.id_word[pred[i][j]],
			print ""
