...
class Model(object):
	def __init__(self):
		...
		self.data_size = 1450
		self.batch_size = 50
		self.num_steps = 80 		# #(neurons) of input layer 1
		self.num_steps_out = 42 + 2 # #(neurons) of input layer 1
		self.dim_size = 4096 		# #(neurons) of input layer 2
		self.n_layers = 1 		 
		...
		self.vocab_size = 5993 + 2 	# #(neurons) of output layer (class)
		self.is_train = 1400
	...
	def nn_test_model(self):
		...
		with tf.variable_scope("train"):
			tf.get_variable_scope().reuse_variables()
			tvars = tf.trainable_variables()
			grads, _ = tf.clip_by_global_norm(tf.gradients(self.loss_test, tvars), 5)
			optimizer = tf.train.AdamOptimizer(self.lr)
			self.train_step_2 = optimizer.apply_gradients(zip(grads, tvars))
	...
	def load_testing(self):
		self.id_word = pickle.load(open("../model/id_word", "rb"))
		self.x_test = np.load("../testing_src/feat_test.npy")
		self.sampling = 2
	...
	def ans_all(self):
		...
		for i in range(10):
			for j in range(5): self.one_epoch()
			pred = self.testing()
			'''for j in range(50):
				t = []
